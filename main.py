...
def YtConvert(VideoURL,FileFormat2):
    try:
        ...
        video.streams.filter(progressive=True,file_extension=str(FileFormat2)).order_by('resolution').desc().first().download()
